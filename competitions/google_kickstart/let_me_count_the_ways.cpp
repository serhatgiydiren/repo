...
ll nCkMp(ll n, ll k) { D(n,k); ll numerator = 1,denominator = 1;for (int i=0; i<k; i++) numerator = (numerator * (n-i) ) % MDL;for (int i=1; i<=k; i++) denominator = (denominator * i) % MDL;return (numerator*modinv(denominator)) % MDL; }
...
ll nCrModPFermat(int n, int r)
{ 
 if (r==0) return 1; 
 return (myfact[n]*modinv(myfact[r],MDL)%MDL*modinv(myfact[n-r],MDL)%MDL)%MDL; 
} 

void init()
{
 mypow[0]=1;
 FOR(i,1,MAXN) mypow[i]=(mypow[i-1]*2)%MDL;
 myfact[0]=1;
 FOR(i,1,2*MAXN) myfact[i]=(myfact[i-1]*i)%MDL;
}
...
ll res=0,tres;
 ...
 FOR(i,0,m+1)
 {
  tres=(nCrModPFermat(m,i)*mypow[i])%MDL;
  tres=(tres*myfact[2*n-i])%MDL;
  res=(res+c*tres)%MDL,c*=-1;
  res%=MDL;
  res=(res+MDL)%MDL;
  //D(c,n,i,nCkMp(n,i),ll(pow(2,i)),fact(2*n-i),res);
 }
