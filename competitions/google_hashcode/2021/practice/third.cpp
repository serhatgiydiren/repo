...
int m,t2,t3,t4,m_used=0;
vector < team > all_teams,best_teams;
vector < pizza > p;
int team_id=0,restart=10;
...
void type_0()
{
 ...
 if (is_accepted(diff)) score_curr+=diff,all_teams[idx1]=bck1,all_teams[idx2]=bck2,D("t0",restart,temperature,thres,diff,score_curr,score_best);
}

void type_1()
{
 if (m_used==m) return;
 int idx1=rand()%team_id;
 int idx1_swap=rand()%all_teams[idx1].c;
 int idx2_swap=rand()%unused.c;
 auto score_old=score(idx1);

 auto bck1=all_teams[idx1];
 
 for(auto e:p[bck1.pid[idx1_swap]].in) if (--bck1.m[e]==0) bck1.d--;
 swap(bck1.pid[idx1_swap],unused.pid[idx2_swap]);
 for(auto e:p[bck1.pid[idx1_swap]].in) if (bck1.m[e]++==0) bck1.d++;

 auto diff=score(bck1)-score_old;
 if (is_accepted(diff)) score_curr+=diff,all_teams[idx1]=bck1,D("t1",restart,temperature,thres,diff,score_curr,score_best);
 else swap(bck1.pid[idx1_swap],unused.pid[idx2_swap]);
}
...
void prep()
{
 ...
 auto backup=p;
 while(team_id<(int)all_teams.size())
 {
  int best_idx=-1;
  ...
  {
   int c=0;
   for(auto e:backup[i].in) if (all_teams[team_id].m.find(e)==all_teams[team_id].m.end()) c++;
   if (c>best_score)
   {
    best_score=c;
    best_idx=i;
  ...
  D(team_id,best_idx,best_score,m_used,all_teams.size());
  assert(best_idx!=-1);
  add_p_to_t(backup[best_idx].id, team_id);
  backup.erase(backup.begin()+best_idx);
  if (++m_used==m) break;
  if ((int)all_teams[team_id].pid.size()==all_teams[team_id].c) team_id++;
 }
 for(auto e:backup) unused.pid.push_back(e.id);
