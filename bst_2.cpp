...
void do_inorder(struct node* root)
{
...
void do_preorder(struct node* root)
{
...
void do_postorder(struct node* root)
{
 cout << "Postorder traversal : " << flush;
 postorder(root);
 wl(flush);
}
...
int n,q;
 ...
 {
  r(q);
  if (i==0) root=insert(root, q);
  else insert(root, q);
 }
 do_inorder(root);
 do_preorder(root);
 do_postorder(root);
 wl("after inserting the node value 100....");
 insert(root, 100);
 do_inorder(root);
 do_preorder(root);
 do_postorder(root);
 wl("after deleting the node value 100....");
