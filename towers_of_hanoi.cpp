...
//The minimum number of moves = 2^n-1 moves
void towers_of_hanoi(int count, char source, char destination, char intermediate)
{
 if (count==1) printf("Move top disc (1) from pole %c to pole %c\n", source, destination);
 else
 {
  towers_of_hanoi(count-1, source, intermediate, destination);
  printf("Move top disc (%d) from pole %c to pole %c\n", count, source, destination);
  towers_of_hanoi(count-1, intermediate, destination, source);
 }
}
