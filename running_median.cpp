...
void add(int a)
{
 (sz(max_heap) && a>=max_heap.top())?min_heap.push(a):max_heap.push(a);
 if (sz(max_heap)-sz(min_heap)>1)
 {
  int temp=max_heap.top();
  max_heap.pop();
  min_heap.push(temp);
 }
 else if (sz(min_heap)-sz(max_heap)>1)
 {
...
double get_median()
{
 if ((sz(min_heap)+sz(max_heap))%2) return ((sz(max_heap)>sz(min_heap))?max_heap.top():min_heap.top());
