...
typedef struct { int a,b,c; } e;

bool cmp(e x,e y)
{
 if (x.a/block!=y.a/block) return x.a/block<y.a/block;
 return x.b<y.b; 
}

void solve() {t1=clock();
int n,t,cur_left=0,cur_right=0;
ll pr=0;
 ...
 {
  res[es[i].c]=pr;
  while(cur_right<es[i].b)
  {
   cur_right++;
   c[x[cur_right]]++;
   res[es[i].c]+=1ll*x[cur_right]*(2*c[x[cur_right]]-1);
  }
  while(cur_right>es[i].b)
  {
   res[es[i].c]-=1ll*x[cur_right]*(2*c[x[cur_right]]-1);
  ...
  while(cur_left<es[i].a)
  {
   res[es[i].c]-=1ll*x[cur_left]*(2*c[x[cur_left]]-1);
   c[x[cur_left]]--;
   cur_left++;
  }
  while(cur_left>es[i].a)
