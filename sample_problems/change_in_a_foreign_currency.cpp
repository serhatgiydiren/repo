...
bool canGetExactChange(const int &targetMoney, const vector < int > &denominations)
{
 vector < bool > x(targetMoney+1);
 x[0]=true;
 for(int i=1;i<=targetMoney;i++)
  for(size_t j=0;j<denominations.size();j++) 
   if (denominations[j]<=i && x[i-denominations[j]]) { x[i]=true; break; }
